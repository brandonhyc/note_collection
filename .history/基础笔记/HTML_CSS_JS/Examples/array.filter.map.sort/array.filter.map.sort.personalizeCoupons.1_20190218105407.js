function personalizeCoupons(coupons, preferredCategories) {
    ...
    let selectedCoupons = coupons.filter(coupon =>  preferredCategories.includes(coupon.category))
    selectedCoupons.sort((a, b) => {
        return b.couponAmount / b.itemPrice - a.couponAmount / a.itemPrice;
    })
    selectedCoupons = selectedCoupons.slice(0, 10);
    selectedCoupons = selectedCoupons.map(coupon => {
            upc: coupon.upc,
